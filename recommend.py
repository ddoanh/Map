...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    "*** YOUR CODE HERE ***"
    cl_lst = [[find_closest(restaurant_location(i), centroids), i] for i in restaurants]
    return group_by_first(cl_lst)
    # END Question 4
...
def k_means(restaurants, k, max_updates=100):
    ...
    while previous_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        "*** YOUR CODE HERE ***"
        grouped_restaurants = group_by_centroid(restaurants, previous_centroids)
        centroids = [find_centroid(r) for r in grouped_restaurants]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    var1 = []
    for x in xs:
        var1.append(x - mean(xs))

    var2 = []
    for y in ys:
        var2.append(y - mean(ys))
        
    S_xx = sum([(e)**2 for e in var1])
    S_yy = sum([(i)**2 for i in var2])
    
    product = []
    for e in range(len(var1)):
        product.append(var1[e]*var2[e])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    "*** YOUR CODE HERE ***"
    find_pred = []
    for r in feature_fns:
        find_pred.append(find_predictor(user, reviewed, r))
    return max(find_pred, key = lambda y:y[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for i in restaurants:
        if i in reviewed:
            d[restaurant_name(i)] = user_score(user, restaurant_name(i))
        else:
